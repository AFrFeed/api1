...
module.exports.controller = (app, io, socket_list) => {
    ...
    const msg_success = "успешно";
    const msg_fail = "ошибка";
    const msg_invalidUser = "неверные имя пользователя и пароль";
    const msg_already_register = "эта электронная почта уже зарегистрировано ";
    const msg_added_favorite = "успешно добавлен список избранных";
    const msg_removed_favorite = "успешно удален список избранного";
    const msg_invalid_item = "недопустимый товар";
    const msg_add_to_item = "товар успешно добавлен в корзину ";
    const msg_remove_to_cart = "товар успешно удален из корзины формы"
    const msg_add_address = "адрес успешно добавлен"
    const msg_update_address = "адрес успешно обновлен"
    const msg_remove_address = "адрес успешно удален"

    const msg_add_payment_method = "способ оплаты успешно добавлен"
    const msg_remove_payment_method = "способ оплаты успешно удален"
    ...
    app.post('/api/app/login', (req, res) => {
        ...
        helper.CheckParameterValid(res, reqObj, ["email", "password", "dervice_token"], () => {
            ...
            db.query("UPDATE `user_detail` SET `auth_token`= ?,`dervice_token`=?,`modify_date`= NOW() WHERE `user_type` = ? AND `email` = ? AND `password` = ? AND `status` = ?", 
                [auth_token, reqObj.dervice_token, "1", reqObj.email, reqObj.password, "1"], (err, result) => {
                ...
                if (result.affectedRows > 0) {
                    ...
                    db.query('SELECT `user_id`, `username`, `name`, `email`, `mobile`, `mobile_code`,`auth_token`, `status`, `created_date` FROM `user_detail` WHERE `email` = ? AND `password` = ? AND `status` = "1" ', 
    ...
    app.post('/api/app/home', (req, res) => {
        ...
        checkAccessToken(req.headers, res, (uObj) => {
            ...
            db.query("SELECT `od`.`price` as `offer_price`, `od`.`start_date`, `od`.`end_date`, `pd`.`prod_id`, `pd`.`cat_id`, `pd`.`brand_id`, `pd`.`type_id`, `pd`.`name`, `pd`.`detail`, `pd`.`unit_name`, `pd`.`unit_value`, `pd`.`nutrition_weight`, `pd`.`price`,(CASE WHEN `imd`.`image` != '' THEN  CONCAT( '" + image_base_url + "' ,'', `imd`.`image` ) ELSE '' END) AS `image`  , `cd`.`cat_name`,  `td`.`type_name`, ( CASE WHEN `fd`.`fav_id` IS NOT NULL THEN 1 ELSE 0 END ) AS `is_fav`, AVG( CASE WHEN `rd`.`rate` IS NOT NULL THEN `rd`.`rate` ELSE 0 END ) AS `avg_rating` FROM `offer_detail` AS `od` " +
                ...
                "LEFT JOIN `review_detail` AS `rd` ON `rd`.`prod_id` = `pd`.`prod_id` " +
                "INNER JOIN `type_detail` AS `td` ON `pd`.`type_id` = `td`.`type_id` AND `td`.`status` = 1 " +
                "WHERE `pd`.`status` = ? GROUP BY `pd`.`prod_id` ORDER BY `pd`.`prod_id` DESC LIMIT 50 ;", [
    ...
    app.post('/api/app/search', (req, res) => {
        helper.Dlog(req.body);
        
        var reqObj = req.body;
        var searchQuery = reqObj.search_query || '';  // Текст запроса для поиска товаров
        ...
        checkAccessToken(req.headers, res, (userObj) => {
            ...
            var likeSearchQuery = `%${searchQuery}%`; // Используем LIKE для поиска по подстроке
            console.log("Formatted LIKE search query:", likeSearchQuery);
    
            var sqlQuery = `
                SELECT pd.prod_id, pd.cat_id, pd.brand_id, pd.type_id, pd.name, pd.detail, pd.unit_name, 
                       pd.unit_value, pd.nutrition_weight, pd.price, 
                       (CASE WHEN imd.image != '' THEN CONCAT( ?, '', imd.image) ELSE '' END) AS image, 
                       cd.cat_name, td.type_name, 
                       (CASE WHEN fd.fav_id IS NOT NULL THEN 1 ELSE 0 END) AS is_fav, 
                       AVG(CASE WHEN rd.rate IS NOT NULL THEN rd.rate ELSE 0 END) AS avg_rating
                ...
                LEFT JOIN review_detail AS rd ON rd.prod_id = pd.prod_id
                INNER JOIN type_detail AS td ON pd.type_id = td.type_id AND td.status = 1
                WHERE pd.status = 1 AND pd.name LIKE ?
                GROUP BY pd.prod_id
                ORDER BY pd.prod_id DESC;
            `;
    
            db.query(sqlQuery, [image_base_url, userObj.user_id, likeSearchQuery], (err, result) => {
                if (err) {
                ...
                res.json({
                    "status": "1",
                    "payload": {
                        "search_results": result,  // Возвращаем результаты поиска
                    },
    ...
    app.post('/api/app/add_delivery_address', (req, res) => {
        ...
        var reqObj = req.body;
        checkAccessToken(req.headers, res, (userObj) => {
            helper.CheckParameterValid(res, reqObj, ["name", "type_name", "phone", "address", "city", "state", "postal_code", "latitude", "longitude"], () => {
                // Добавление адреса в базу данных, включая координаты
                db.query("INSERT INTO `address_detail`(`user_id`, `name`, `phone`, `address`, `city`, `state`, `type_name`, `postal_code`, `latitude`, `longitude`) VALUES (?,?,?,?,?,?,?,?,?,?)", 
                [userObj.user_id, reqObj.name, reqObj.phone, reqObj.address, reqObj.city, reqObj.state, reqObj.type_name, reqObj.postal_code, reqObj.latitude, reqObj.longitude], (err, result) => {
                    if (err) {
                        helper.ThrowHtmlError(err, res);
                        return;
                    }
    ...
    app.post('/api/app/update_delivery_address', (req, res) => {
        ...
        var reqObj = req.body;
        checkAccessToken(req.headers, res, (userObj) => {
            helper.CheckParameterValid(res, reqObj, ["address_id", "name", "type_name", "phone", "address", "city", "state", "postal_code", "latitude", "longitude"], () => {
                // Обновление адреса с координатами
                db.query("UPDATE `address_detail` SET `name`=?, `phone`=?, `address`=?, `city`=?, `state`=?, `type_name`=?, `postal_code`=?, `latitude`=?, `longitude`=?, `modify_date`= NOW() WHERE `user_id` = ? AND `address_id` = ? AND `status` = 1", 
                [reqObj.name, reqObj.phone, reqObj.address, reqObj.city, reqObj.state, reqObj.type_name, reqObj.postal_code, reqObj.latitude, reqObj.longitude, userObj.user_id, reqObj.address_id], 
                (err, result) => {
                    if (err) {
                        helper.ThrowHtmlError(err, res);
                        return;
    ...
    app.post('/api/app/delete_delivery_address', (req, res) => {
        ...
        checkAccessToken(req.headers, res, (userObj) => {
            helper.CheckParameterValid(res, reqObj, ["address_id"], () => {
                db.query("UPDATE `address_detail` SET `status`=2, `modify_date`= NOW() WHERE `user_id` = ? AND `address_id` = ? AND `status` = 1", 
                [userObj.user_id, reqObj.address_id], (err, result) => {
                    if (err) {
                        helper.ThrowHtmlError(err, res);
                        return;
                    }
    ...
    app.post('/api/app/delivery_address', (req, res) => {
        ...
        var reqObj = req.body;
        checkAccessToken(req.headers, res, (userObj) => {
            db.query("SELECT `address_id`, `name`, `phone`, `address`, `city`, `state`, `type_name`, `postal_code`, `latitude`, `longitude`, `is_default` FROM `address_detail` WHERE `user_id` = ? AND `status` = 1", 
